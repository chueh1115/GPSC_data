...
File<- paste(DataDir,"GS1_POC_r2.input",sep="")  
LIM<- Setup(file=File) 
#Parsimonious####
...
plotweb(Flowmatrix(LIM),main="GS1",
...
xs <- xsample(E    = LIM$A,
              ...
              x0   = x0,
              iter = iter)
nameoutput <- "GS1_POC_r2_10000_100.Rdata" #1000=iteration
...
load("GS1_POC_r2_10000_100.Rdata")
LA<-data.frame(flow=LIM$Unknowns, 
               mean=colMeans(xs$X),
               sd=sqrt(diag(var(xs$X))))
...
png(paste("Carbon flows of GS1.png",sep = "_"))
name<-LIM$Unknowns
